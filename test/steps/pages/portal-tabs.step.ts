...
When(
  "the user clicks on the {string} tab",
  async ({ homePage, portalContext }, tab: string) => {
...
When(
  "the user hovers over the {string} tab",
  async ({ homePage, portalContext }, tab: string) => {
    portalContext.selectedTab = tab;
...
When("the user hovers over a tab", async ({ homePage, portalContext }) => {
  if (!portalContext.selectedTab)
    throw new Error("❌ No tab selected to hover over");
  await homePage.header.hoverTab(portalContext.selectedTab);
});

When(
  "the corresponding dropdown menu is displayed",
  async ({ homePage, portalContext }) => {
...
When(
  "the user clicks on the {string} option",
  async ({ homePage, portalContext }, option: string) => {
    if (!portalContext.selectedTab)
      throw new Error("❌ No tab selected before submenu click");
...
When(
  "the user switches to and verifies the {string} tab",
  async ({ homePage, portalContext }, tab: string) => {
    portalContext.selectedTab = tab;
...
When(
  "the user switches from {string} to {string}",
  async ({ homePage, portalContext }, previousTab: string, newTab: string) => {
...
Then(
  "the {string} submenu option should be displayed",
  async ({ homePage, portalContext }, option: string) => {
    if (!portalContext.selectedTab) throw new Error("❌ No tab selected");
    await homePage.header.verifySubMenuVisible(
      portalContext.selectedTab,
      option,
    );
  },
);
...
Then(
  "the selected tab should be highlighted as active",
  async ({ homePage, portalContext }) => {
    if (!portalContext.selectedTab) throw new Error("❌ No tab selected");
...
Then(
  "the {string} tab should redirect to its product listing page",
  async ({ homePage }, tab: string) => {
...
Then(
  "the {string} tab should be highlighted as active",
  async ({ homePage }, tab: string) => {
...
Then(
  "the dropdown menu should no longer be displayed",
  async ({ homePage, portalContext }) => {
    if (!portalContext.selectedTab) throw new Error("❌ No tab selected");
