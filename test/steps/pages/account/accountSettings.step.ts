...
When(
  "the user updates the {string} field with valid data",
  async ({ accountSettingsPage }, fieldName: string) => {
...
When(
  "the user has updated valid account details:",
  async ({ accountSettingsPage }, table: DataTable) => {
...
When(
  ...
  async ({ accountSettingsPage }, buttonName: string) => {
...
Then(
  ...
  async ({ loginPage, accountInfoPage }) => {
...
Then(
  ...
  async ({ accountSettingsPage }, url: string) => {
...
Then("the {string} link should be visible", async ({ accountSettingsPage }) => {
...
Then(
  "the following sections should be displayed:",
  async ({ accountSettingsPage }, dataTable: DataTable) => {
    const entries = dataTable.hashes();
...
Then(
  "the following action buttons should be visible:",
  async ({ accountSettingsPage }, dataTable: DataTable) => {
...
Then(
  "the {string} field should reflect the updated value",
  async ({ accountSettingsPage }, fieldName: string) => {
...
Then("the changes should not be saved", async ({ accountInfoPage }) => {
  await accountInfoPage.verifyAccountInfoFieldValue(
    AccountInfoFields.FirstName,
    accountInfo.firstName,
  );
  await accountInfoPage.verifyAccountInfoFieldValue(
    AccountInfoFields.LastName,
    accountInfo.lastName,
  );
  await accountInfoPage.verifyAccountInfoFieldValue(
    AccountInfoFields.Phone,
    accountInfo.phone,
  );
  await accountInfoPage.verifyAccountInfoFieldValue(
    AccountInfoFields.Email,
    accountInfo.email,
  );
  await accountInfoPage.verifyAccountInfoFieldValue(
    AccountInfoFields.EmailSignUp,
    accountInfo.emailSignUp,
  );
});
