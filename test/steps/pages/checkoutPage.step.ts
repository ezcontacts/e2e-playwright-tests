...
Then(
  "the selected shipping address should be displayed",
  async ({ checkoutPage }) => {
...
Then(
  "the shipping options applicable to {string} should be displayed",
  async ({ checkoutPage }, country) => {
    await checkoutPage.verifyShippingOptionsForCountry(country, 0);
  },
);

Then(
  "the user should be able to open available shipping options",
  async ({ checkoutPage }) => {
...
Then(
  "the user should be able to select one of the available shipping options",
  async ({ checkoutPage }) => {
...
Then(
  "the user should be able to select created shipping options",
  async ({ checkoutPage, addressAndPaymentPage }) => {
    selectedShippingOptionText =
...
Then(
  "the default shipping option should remain selected",
  async ({ checkoutPage }) => {
...
When(
  "the user updates the shipping address country to {string}",
  async ({ checkoutPage }, country: string) => {
    previousShippingCountry = await checkoutPage.updateShippingAddressCountry(
...
Then(
  "the previously displayed shipping options should no longer be available",
  async ({ checkoutPage }) => {
