...
Then(
  "the page should display a message {string}",
  async ({ wishlistEmptyPage }, message: string) => {
    await wishlistEmptyPage.verifyMessageVisible(message);
  },
);

Then(
  "the page should display the following links:",
  async ({ wishlistEmptyPage }, dataTable: DataTable) => {
