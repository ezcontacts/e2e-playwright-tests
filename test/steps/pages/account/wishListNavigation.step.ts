...
Given(
  ...
  async ({ wishListNavigationPage }, pageName: string) => {
    await wishListNavigationPage.open();
  },
);

Then(
  "the page should display the title {string}",
  async ({ wishListNavigationPage }, text: string) => {
    await wishListNavigationPage.verifyTitleHaveText(text);
  },
);

Then(
  "the page should display the heading {string}",
  async ({ wishListNavigationPage }, text: string) => {
    await wishListNavigationPage.verifyHeadingText(text);
  },
);
