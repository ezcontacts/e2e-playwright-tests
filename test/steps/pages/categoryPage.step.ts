...
Given(
  "the user views a product with no reviews on the listing page",
...
When(
  "the user refreshes the Product Listing page",
  async ({ sunglassesPage }) => {
...
When(
  "the user selects {string} from the Products per page dropdown",
...
When(
  "the user selects {string} from the Sort By dropdown",
  async ({ sunglassesPage }, text: string) => {
    await sunglassesPage.setProductMatchDropdownValue(text);
    await sunglassesPage.verifyProductMatchDropdownIsHaveValue(
      FILTER_VALUE[text].value,
    );
  },
);

When("set first rating filter", async ({ sunglassesPage }) => {
  await sunglassesPage.promotion.closeDynamicPopupIfPresent();
  // await sunglassesPage.fillter.clickOnFirstRatingFilter();
});

Then(
  "the Products per page dropdown should be visible",
  async ({ sunglassesPage }) => {
...
Then(
  "the default 30 products per page value should be selected",
  async ({ sunglassesPage }) => {
...
Then("the Sort By dropdown should be visible", async ({ sunglassesPage }) => {
