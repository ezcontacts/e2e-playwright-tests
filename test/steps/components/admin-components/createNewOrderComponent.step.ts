...
When("I enter the customer email", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.enterEmail(CUSTOMER.email);
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I confirm the shipping address", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.verifyTitleIsVisible(
    ADMIN.createOrder.shippingAddressTitle,
  );
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I confirm the billing address", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.verifyTitleIsVisible(
    ADMIN.createOrder.billingAddressTitle,
  );
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I select the payment", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.clickOnFirstPaymentMethod();
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I do not select the payment method", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I select the shipping method", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.clickOnNextBtn();
});

When("I add a new item to the existing order", async ({ adminPanelPage }) => {
  await adminPanelPage.createNewOrder.clickOnAddProductBtn();
  await adminPanelPage.addProductModelComponent.selectProductByType(
...
When(
  ...
  async ({ adminPanelPage }) => {
    ...
    await adminPanelPage.createNewOrder.verifyAndHandleDialog(
      "Send payment request now?",
    );
  },
);
