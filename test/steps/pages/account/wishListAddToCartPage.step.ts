...
Then('I should see the product added to the wish list', async ({wishListAddToCartPage}) => {
  await wishListAddToCartPage.verifyProductAddedToWishList();
});

When('the user clicks the "Add to Cart" link for the product', async ({ wishListAddToCartPage }) => {
  await wishListAddToCartPage.clickAddToCartForFirstProduct();
});

Then("the product should be added to the cart", async ({ wishListAddToCartPage }) => {
  await wishListAddToCartPage.verifyProductAddedToCart();
});

Then("the user should be redirected to the Cart page", async ({ wishListAddToCartPage }) => {
