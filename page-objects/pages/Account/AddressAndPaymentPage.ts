...
export class AddressAndPaymentPage extends AccountPage {
  ...
  constructor(page: Page) {
    ...
    this.AddLink = (type: string) =>
      page.getByRole("link", { name: `Add ${type}` });
    this.Heading = (name: string) => page.getByRole("heading", { name: name });
  }

  async clickAddLink(type: string): Promise<void> {
    await this.AddLink(type).click();
  }

  async verifyFormDisplayed(formName: string): Promise<void> {
    const heading = this.Heading(formName);
